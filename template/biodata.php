<!DOCTYPE html>
<html lang="id">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Biodata</title>
  <link rel="stylesheet" href="biodata.css">
</head>

<body>
  <div class="biodata-mahasiswa">
    <div class="rectangle-63"></div>
    <div class="rectangle-38"></div>
    <div class="logout">LOGOUT</div>
    <div class="rectangle-11"></div>
    <div class="rectangle-36"></div>
    <div class="rectangle-37"></div>
    <div class="biodata">Biodata</div>
    <div class="rectangle-95"></div>
    <div class="informasi-mahasiswa">Informasi Mahasiswa</div>
    <div class="tempat-lahir-tanggal-lahir-umur-alamat-jenis-kelamin-agama">
      <span>
        <ul
          class="tempat-lahir-tanggal-lahir-umur-alamat-jenis-kelamin-agama-span">
          <li>Tempat Lahir</li>
          <br />
          <li>Tanggal Lahir</li>
          <br />
          <li>Umur</li>
          <br />
          <li>Alamat</li>
          <br />
          <li>Jenis Kelamin</li>
          <br />
          <li>Agama</li>
          <br />

        </ul>
      </span>
    </div>
    <div
      class="cirebon-bandung-14-juli-2004-20-tahun-jl-tlogomas-laki-laki-atheis">
      Cirebon, Bandung
      <br />
      <br />
      14 Juli 2004
      <br />
      <br />
      20 Tahun
      <br />
      <br />
      JL. Tlogomas
      <br />
      <br />
      Laki - Laki
      <br />
      <br />
      Atheis
    </div>
    <div class="informasi-keluarga">Informasi Keluarga</div>
    <div class="nama-ayah-nama-ibu-nik-ayah-nik-ibu">
      <span>
        <ul class="nama-ayah-nama-ibu-nik-ayah-nik-ibu-span">
          <li>Nama Ayah</li>
          <br />
          <li>Nama Ibu</li>
          <br />
          <li>NIK Ayah</li>
          <br />
          <li>NIK Ibu</li>
          <br />

        </ul>
      </span>
    </div>
    <div class="agus-siti-0129301238127372-1230812308212222">
      Agus
      <br />
      <br />
      Siti
      <br />
      <br />
      0129301238127372
      <br />
      <br />
      1230812308212222
      <br />
    </div>
    <div class="informasi-domisili">Informasi Domisili</div>
    <div class="alamat-rt-rw-kelurahan-kecamatan-kota-kode-pos">
      <span>
        <ul class="alamat-rt-rw-kelurahan-kecamatan-kota-kode-pos-span">
          <li>Alamat</li>
          <br />
          <li>RT/RW</li>
          <br />
          <li>Kelurahan</li>
          <br />
          <li>Kecamatan</li>
          <br />
          <li>Kota</li>
          <br />
          <li>Kode Pos</li>
          <br />

        </ul>
      </span>
    </div>
    <div class="jl-tlogomas-04-06-merjosari-lowokwaru-malang-65144">
      JL. Tlogomas
      <br />
      <br />
      04/06
      <br />
      <br />
      Merjosari
      <br />
      <br />
      Lowokwaru
      <br />
      <br />
      Malang
      <br />
      <br />
      65144
    </div>

    <div class="biodata-mahasiswa2">Biodata Mahasiswa</div>
    <div class="line-2"></div>
    <div class="rectangle-59"></div>
    <img class="rectangle-48" src="profil.png" />
    <div class="abdul-aziz-2221771013-d-4-teknik-informatika-2-g-02">
      Abdul Aziz
      <br />
      <br />
      2221771013
      <br />
      <br />
      D-4 Teknik Informatika
      <br />
      <br />
      2G / 02
      <br />
      <br />
    </div>
    <img class="image-4" src="image-40.png" />
    <img class="image-5" src="image-50.png" />
    <img class="image-6" src="image-60.png" />
    <div class="kontak">Kontak</div>
    <div class="abdulaziz-gmail-com">abdulaziz@gmail.com</div>
    <div class="_081231272828">081231272828</div>
    <div class="abdul-aziz">Abdul Aziz</div>
    <div class="data-master">Data Master</div>
    <div class="rectangle-83"></div>
    <div class="management-tugas-akhir">Management Tugas Akhir</div>
    <div class="request-bebas-tugas-akhir">Request Bebas Tugas Akhir</div>
    <div class="rectangle-382"></div>
    <div class="rectangle-383"></div>
    <div class="management-dokumen-pendukung">Management Dokumen Pendukung</div>
    <div class="rectangle-384"></div>
    <img class="_103408-text-book-icon-1" src="book-icon-10.png" />
    <img class="book-solid-1" src="book-solid.png" />
    <img class="image-13" src="req.png" />
    <div class="biodata2">Biodata</div>
    <div class="rectangle-385"></div>
    <img class="gauge-solid-1" src="biodata.png" />
    <img
      class="sistem-bebas-tanggungan-tugas-akhir-1-1"
      src="sibeta.png" />
  </div>
</body>

</html>