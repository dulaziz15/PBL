...
class BebasTanggungan
{
    ...
    public function getOneMahasiswa($id) {
        $query = "SELECT * FROM Bebas_tanggungan as bt inner join Tugas_akhir as ta on ta.tugas_akhir_id = bt.tugas_akhir_id inner join Mahasiswa as mhs on mhs.mahasiswa_id = ta.mahasiswa_id where mhs.mahasiswa_id = $id";
        $data = $this->koneksi->KoneksiDB()->query($query);
        $result = $data->fetch();
        return $result;
    }
